...
watch(input_Email, (newValue) => {
  ...
  if (input_Email.value == "")
    return (email_check.value = { EmailAlread: true, EmailFormat: true });
  ...
  if (emailPattern.test(newValue)) {
    ...
    axios
      ...
      .catch((err) => {
        ...
        if (result.err_Code == "EmailAlreadyExists")
          email_check.value.EmailAlread = false;
      });
  } else {
...
const singup_Button_Click = () => {
  if (input_Pwd.value !== input_Pwd_Ch.value)
    return console.log("비밀번호 불일치");
  if (!email_check.value.EmailAlread || !email_check.value.EmailFormat)
    return console("이메일 인증 문제");
  axios
    .post("/users/signup", {
