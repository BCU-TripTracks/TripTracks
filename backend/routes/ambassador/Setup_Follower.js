...
router.use("/", async (req, res, next) => {
  ...
  try {
    ...
    let [Now_Month_Info_Log] = await conn.query(
      "SELECT Follower FROM Ambass_Info_Log WHERE User_ID=? AND Year=YEAR(NOW()) AND Month=MONTH(NOW())",
      [User_ID]
    );
    ...
    let [Last_Month_Info_Log] = await conn.query(
      `SELECT Follower FROM Ambass_Info_Log WHERE User_ID=? AND 
    (Year = YEAR(DATE_SUB(NOW(), INTERVAL 1 MONTH)) AND Month = MONTH(DATE_SUB(NOW(), INTERVAL 1 MONTH)))`,
      [User_ID]
    );
    return res.status(200).json({
      Follower_Info: {
        Now_Follower: Now_Month_Info_Log.Follower,
        Last_Follower: Last_Month_Info_Log ? Last_Month_Info_Log.Follower : 0,
      },
    });
  } catch (error) {
    console.error(error);
