...
router.use("/", async (req, res, next) => {
  ...
  try {
    ...
    let [Now_Month_Info_Log] = await conn.query(
      "SELECT Feed_Like FROM Ambass_Info_Log WHERE User_ID=? AND Year=YEAR(NOW()) AND Month=MONTH(NOW())",
      [User_ID]
    );
    ...
    let [Last_Month_Info_Log] = await conn.query(
      `SELECT Feed_Like FROM Ambass_Info_Log WHERE User_ID=? AND 
    (Year = YEAR(DATE_SUB(NOW(), INTERVAL 1 MONTH)) AND Month = MONTH(DATE_SUB(NOW(), INTERVAL 1 MONTH)))`,
      [User_ID]
    );
    return res.status(200).json({
      Like_Info: {
        Now_Like: Now_Month_Info_Log.Feed_Like,
        Last_Like: Last_Month_Info_Log ? Last_Month_Info_Log.Feed_Like : 0,
      },
    });
  } catch (error) {
    console.error(error);
