...
router.use("/", async (req, res, next) => {
  ...
  try {
    ...
    let [Now_Month_Info_Log] = await conn.query(
      "SELECT Score FROM Ambass_Info_Log WHERE User_ID=? AND Year=YEAR(NOW()) AND Month=MONTH(NOW())",
      [User_ID]
    );
    ...
    let [Last_Month_Info_Log] = await conn.query(
      `SELECT Score FROM Ambass_Info_Log WHERE User_ID=? AND 
    (Year = YEAR(DATE_SUB(NOW(), INTERVAL 1 MONTH)) AND Month = MONTH(DATE_SUB(NOW(), INTERVAL 1 MONTH)))`,
      [User_ID]
    );
    return res.status(200).json({
      Score_Info: {
        Now_Score: Now_Month_Info_Log.Score,
        Last_Score: Last_Month_Info_Log ? Last_Month_Info_Log.Score : 0,
      },
    });
  } catch (error) {
    console.error(error);
