...
router.post("/add", async (req, res) => {
  ...
  try {
    ...
    if (authorUserRule === 1) {
      ...
      const insertPostSaveQuery = "INSERT INTO Post_Save (User_ID, Post_ID) VALUES (?, ?)";
      await conn.query(insertPostSaveQuery, [userId, postId]);
    } else if (authorUserRule === 2) {
      // 게시글 작성자가 홍보대사(Ambass)일 경우 Ambass_Save 테이블에 저장
      const insertAmbassSaveQuery = "INSERT INTO Ambass_Save (User_ID, Post_ID) VALUES (?, ?)";
      await conn.query(insertAmbassSaveQuery, [userId, postId]);
    } else {
      return res.status(400).json({ error: "잘못된 게시글 작성자 유형입니다." });
    }

    await conn.commit(); // 트랜잭션 커밋
    return res.status(200).json({ message: "게시물이 성공적으로 저장되었습니다." });
  } catch (error) {
    console.error(error);
...
router.post("/delete", async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({ message: "저장된 게시물이 성공적으로 삭제되었습니다." });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ error: "내부 서버 오류가 발생했습니다." });
  } finally {
